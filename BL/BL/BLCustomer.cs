                    ...
                    foreach (var parcel in dalParcelsList)
                    {
                        if (customerId == parcel.SenderId || customerId == parcel.TargetId)
                        {
                            ...
                            if (parcel.Delivered != null)
                                ParcelInCustomer.ParcelStatus = ParcelStatuses.Delivered;
                            else if (parcel.PickedUp != null)
                                ParcelInCustomer.ParcelStatus = ParcelStatuses.PickedUp;
                            else if (parcel.Scheduled != null)
                                ParcelInCustomer.ParcelStatus = ParcelStatuses.Scheduled;
                            else
                                ParcelInCustomer.ParcelStatus = ParcelStatuses.Requested;
        ...
        public void DeleteCustomer(int customerId)
        {
            try
            {
                ...
                var parcelList1 = from parcel in customer.ParcelFromCustomerList
                                  where parcel.ParcelStatus == ParcelStatuses.Scheduled ||
                                          parcel.ParcelStatus == ParcelStatuses.PickedUp
                                  select parcel;
